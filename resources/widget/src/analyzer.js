...
export function analyzeFrame(frame, overlayBox) {
  // const isInsideBox = detectIDInBox(frame, overlayBox);

  // if (!isInsideBox) {
  //   return { ok: false, message: "Position ID inside box" };
  // }
  const brightness = calcBrightness(frame);
  const motion = calcMotion(frame);

  if (brightness < 60) return { ok: false, message: "Too dark", insideBox: true };
  if (brightness > 220) return { ok: false, message: "Too bright", insideBox: true };
  if (motion > 25) return { ok: false, message: "Hold still", insideBox: true };

  return { ok: true, message: "Hold steady", insideBox: true };
}
